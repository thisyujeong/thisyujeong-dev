...
리액트에서 여러개의 인풋을 다룰 때, 흔히 하나의 콜백함수로 모든 인풋의 상태값을 관리하는 방법을 자주 사용할 것이다.
따라서 다음과 같이 작성한다면 코드가 굉장히 깔끔해진다.
...
하지만 이 방법은 하나의 인풋값만 변경되어도 모든 input이 리렌더링되는데, 필자는 이게 너무 신경쓰이는 것이다..
이 문제는 useRef를 사용하여 개선할 수 있었다.

## useRef로 Input 상태관리 - 리렌더링 최적화

useRef는 리렌더링이 필요한 요소에 사용하면 리렌더링이 되지 않으므로 꼭 리렌더링이 필요한 값에 사용해야한다.
