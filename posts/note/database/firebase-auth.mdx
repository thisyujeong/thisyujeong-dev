...
### 인증 상태 지속성 설정 (Persistence)

[공식 문서](https://firebase.google.com/docs/auth/web/auth-state-persistence)

- `local`(default): 브라우저를 닫더라도 사용자 정보를 기억한다.
- `session`: 탭이 열려있는 동안에는 사용자 정보를 기억한다.
- `none`: 사용자 정보를 기억하지 않는다. 로그인을 시ㄴ도할 경우 로그인은 되지만, 새로고침 시 로그아웃된다.

기본적으로 **Persistance**를 설정하지 않으면 `local`로 설정된다.
