...
이를 개선하기 위해 Debounce를 사용하여, 연속적으로 발생하는 이벤트의 마지막 이벤트를 기점으로 네트워크 요청을 하도록 개선할 수 있다.
...
번외로 디바운스와 유사한, 유용하게 사용할 수 있는 성능 개선 방법 중 throttle이 있다. debounce는 이벤트 발생이 멈출 때 처리한다면, throttle 은 이벤트가 지속적으로 발생하더라고 일정한 간격으로 끊어서 처리한다.
...
- debounce: 일련의 연속된 이벤트에서 마지막 이벤트만 처리하도록 제어하는 기능으로, 사용자 입력과 같은 번번한 이벤트를 처리할 때 사용한다.
- throttling: 정해진 일정 간격으로 요청을 처리한다. 따라서 정기적으로 실행해야한다면 throttle로 처리하는 것이 적합하다.

## 개선할 검색기능 코드

다음 코드는 swr한 코드로, `input` 태그에 `onChange` 이벤트가 발생할 때마다 swr이 새로운 api 요청을 보내므로 성능에 좋지 않다.
...
useEffect를 사용하여 커스텀 훅을 만들어준다. useDebounce 훅의 인자로 value와, delay로 시간을 지정해준다.
value와 delay가 변경될 경우 `setTimeout` 실행하여 지정한 delay 시간이 되면 debounced 상태값을 변경해준다.
만약, setTimeout이 끝나지 않은 이전 handler들은 clearTimeout으로 제거하여 마지막 요청만 실행될 수 있도록 한다.
